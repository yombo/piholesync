...
if [ "$1" == "full" ]; then
  ...
  # Comment out one or more curl lines before if you do not wish to use any of these during the 'full' sync.
...
echo -ne " ${UNTICK} \e[32m Sync files to peer PiHole. \e[0m\033[0K\r"
#LOOP FOR FILE TRANSFER
RESTART=0 # flag determine if service restart is needed
for FILE in ${FILES[@]}
do
  if [[ -f $PIHOLEDIR/$FILE ]]; then
  echo -ne " ${UNTICK} \e[32m Sync files to peer PiHole. Processing file: \e[0m ${FILE} \033[0K\r"
  RSYNC_COMMAND=$(rsync -a --out-format='%i' $PIHOLEDIR/$FILE $HAUSER@$PIHOLE2:$PIHOLEDIR)
    if [ "$RSYNC_COMMAND" != "<f..t......" ] && [ "$RSYNC_COMMAND" != "" ]; then
      ...
      echo -e " ${TICK} \e[32m Sync files to peer PiHole. File changed: \e[0m ${FILE} \033[0K\r"
     # else 
       # no changes
     fi
  # else
...
echo -e " ${TICK} \e[32m Syncing files to peer PiHole. Done.                                    \e[0m"

FILE="adlists.list"
RSYNC_COMMAND=$(rsync -ai $PIHOLEDIR/$FILE $HAUSER@$PIHOLE2:$PIHOLEDIR)
if [[ -n "${RSYNC_COMMAND}" ]]; then
  echo -ne " ${UNTICK} \e[32m adlists.list change detected, running updateGravity on remote peer. This can take a while. \e[0m\033[0K\r"
  ssh $HAUSER@$PIHOLE2 "sudo -S pihole updateGravity >/var/log/pihole_updateGravity.log || cat /var/log/pihole_updateGravity.log"
  echo -e " ${TICK} \e[32m adlists.list change detected, running updateGravity on remote peer.                                    \e[0m"
# else
  # no changes
fi
...
#  sudo -S service pihole-FTL start
  ...
  echo -ne " ${UNTICK} \e[32m Sleeping for ${STARTSLEEPTIME} second for client health. Remaining: \e[0m ${SLEEPTIME} \033[0K\r"
